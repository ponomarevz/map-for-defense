...
//(function() {
	...
	*/
	...
	------------------------------------------------------------------------*/
	function TematikLayerInit(type, name, source, table, hesh_poley) {
		var that = this;
		var  Htype;
		...
		if (type == "point") {
			...
			//------------------ отдаем слой ------------------------------------
			that.layer = new OpenLayers.Layer.Vector(name, {
					...
					var f_a, ft_a;
						...
						//if (feature.sk_Geom_Line) {
							...
							if (feature.sk_Geom_Line_status != 1) {
								...
								var lllline = new OpenLayers.Geometry.LineString(curveOffset(that.layer, feature.geometry.components[0].getVertices(), dx ));
		...
		that.control.edit = new OpenLayers.Control.ModifyFeature(that.layer, { vertexRenderIntent: "temporary", virtualStyle: virtual})
		
		that.control.rotate =  new OpenLayers.Control.RotateGraphicFeature(that.layer); that.control.rotate.mode = OpenLayers.Control.ModifyFeature.ROTATE;
		
		if (type == "point") {
			that.control.draw = new OpenLayers.Control.DrawFeature(that.layer, Htype, { 
		...
		function crateFeature(feature) {
				...
				$('#information1').css({'width': win.width, 'height': win.height})
						.popUp("contentAdd", feature, hesh_poley, "create", "Заполните параметры", function() {
							...
							that.layer.addFeatures(feature);  //----------после сохранения добавляем обїект
							that.layer.redraw();
		...
		function onFeatureSelect(evt) {
			...
			if (fl_red == 1) { 
				feature.state = OpenLayers.State.UPDATE;
				that.control.edit.selectFeature(feature);
				//alert("asdxas");
			}
			if (fl_red == 5) { 
				//feature.state = OpenLayers.State.UPDATE;
				//that.control.rotate.selectFeature(feature);
				//alert("asdxas");
			}
		...
		function onFeatureUnselect(evt) {
			...
			if (fl_red == 1) {
				that.control.edit.unselectFeature(that.control.edit.feature);
			...
			if (fl_red == 5) { 
				//feature.state = OpenLayers.State.UPDATE;
				//that.control.rotate.unselectFeature(that.control.rotate.feature);
				//alert("asdxas");
			}
			if (fl_red == 2) {
				$('#information').hide(); 
		...
		function control_deactivate() {
			that.control.edit.deactivate(); 
			that.control.rotate.deactivate();
			that.control.draw.deactivate();
